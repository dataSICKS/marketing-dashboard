...
export default function SettingsPage() {
  ...
  const handleSave = () => {
    const filtered = adCodes.map((c) => c.trim()).filter(Boolean);
    update({ adCodes: filtered });
  };
            ...
            <div key={i} className="flex items-center gap-2">
              {settingsLoading ? (
                ...
                <input
                  ...
                  placeholder="例: kisekino_haburashi_fb_inst_mid_1_a"
                  className="flex-1 h-9 rounded-lg border px-3 text-sm outline-none transition-colors font-mono"
